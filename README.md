...
- Run built-in commands such as `>desktop`, `>programfiles`, `>appdata`, `>temp`, `>exit`, `>guid`, `>log`, `>refresh`, `>shutdown`, `>logoff`, `>sleep`, and `>lock`.
...
1. Press the launcher hotkey.
   The launcher currently uses `Ctrl+Space`.
2. Start typing.
